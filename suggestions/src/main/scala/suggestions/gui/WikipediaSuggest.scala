...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = (searchTerms map wikiSuggestResponseStream) flatMap { o => o.recovered }
    ...
    val selections: Observable[String] = 
      button.clicks.filter { _ => 
        !suggestionList.selection.items.isEmpty 
      } flatMap { _ => 
        wikiPageResponseStream(suggestionList.selection.items.head) 
      }
