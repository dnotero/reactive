...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs map { unsanitized => unsanitized.replace(' ', '_') }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] =  (obs map { x => Success(x) }) onErrorReturn[Try[T]] { e: Throwable => Failure(e)}
